import base64 as rtfd
import webbrowser
import time
def mikeSwift(cre):
    sto = []
    gre = ""
    for i in cre:
        sto.append(i+str(len(i)))
        sto.append("h4ck" + i)
    # a list of a char + 1, h4ck + char for example for the letter 8 it would be 81, h4ck8
    for i in sto:
        gre+=i
    #turn the list to a string
    return gre

def prompt():
    return bytes(input("Welcome to the loading dock. What is the password?\t"), 'utf-8')

def obfuscate(bys):
    fusc = rtfd.b64encode(bys) #base 64 encoding
    fusc += b"534345fdfgfgfdhty6y56yjl"
    fusc = str(fusc) #makes everything a string
    fusc = fusc[2:len(fusc)-1] #removes unneccessary chars
    refus = []
    for i in fusc:
        refus.append((str(i)))
    #refus will be a list of all chars now
    fusc="florSFIUEfet4565477"
    for i in refus:
        fusc+=i
    #fusc would be florSFIUEfet4565477 + base64(bys) + 534345fdfgfgfdhty6y56yjl
    return fusc

def crypt(sor):
    sro = []
    fusc = "893"
    for i in range(len(sor)):
        sro.append(sor[i]+str(i))
    #sro would be a list of a char and its index for example in 0 we would have f0
    sro.reverse()
    for i in sro:
        fusc+=i
    #fusc would be 893 + reversed version of previous fusc with every letter next to its original index
    return fusc

def grant():
    print("Congartulation. Pleas Procid")
    webbrowser.open("https://ctflearn.com/index.php?action=find_problem_details&problem_id=449")

def punish():
    print("This is going to hurt.")
    while True:
        time.sleep(.1)
        webbrowser.open("https://www.youtube.com/watch?v=O3asoGVHix8")

def main():
    password = prompt()
    sik = obfuscate(password)#sik would be florSFIUEfet4565477 + base64(bys) + 534345fdfgfgfdhty6y56yjl
    print(sik)
    sik = crypt(sik) #sik would be 893 + reversed version of previous fusc with every letter next to its original index
    print(sik)
    sik = mikeSwift(sik)# turns to a string of a char + 1 +h4ck + char
    print(sik)
    if sik == "1h4ck=":
        grant()
    else:
        punish()
main()
